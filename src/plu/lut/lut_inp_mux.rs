...
pub type LUTN_INPX_W<'a, const O: u8> =
    crate::FieldWriter<'a, u32, LUT_INP_MUX_SPEC, u8, LUTN_INPX_A, 6, O>;
impl<'a, const O: u8> LUTN_INPX_W<'a, O> {
...
impl W {
    #[doc = "Bits 0:5 - Selects the input source to be connected to LUT0 input0. For each LUT, the slot associated with the output from LUTn itself is tied low."]
    #[inline(always)]
    pub fn lutn_inpx(&mut self) -> LUTN_INPX_W<0> {
        LUTN_INPX_W::new(self)
    }
